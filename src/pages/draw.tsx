...
export default function Draw(props: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  return (
    <div className="tw-relative">
      <div className="tw-grid tw-w-screen tw-h-screen tw-grid-flow-col tw-p-6 tw-gap-x-10">
        <LeftGrid camera={camera} geogebra={props.geogebra} />
        <RightGrid text={props.text} />
      </div>
...
export const getServerSideProps: GetServerSideProps<{
  ...
  return {
    props: {
      name: (ctx.query.name instanceof Array ? ctx.query.name.join('') : ctx.query.name) ?? 'None',
      text:
        (ctx.query.text instanceof Array ? ctx.query.text.join('') : ctx.query.text) ??
        '새로운 메모!',
      geogebra:
        (ctx.query.geogebra instanceof Array ? ctx.query.geogebra.join('') : ctx.query.geogebra) ??
        defaultGeogebra
    }
  };
};
