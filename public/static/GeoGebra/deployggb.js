...
(function () {
  ...
  var GGBApplet = function () {
    ...
    var fetchParametersFromApi = function (successCallback) {
      var onSuccess = function (text) {
        ...
        views.is3D = true;
        var imageDir = 'static/GeoGebra/images/';
      ...
      var host = location.host.match(/(www|stage|beta|groot|alpha).geogebra.(org|net)/)
        ? location.host
        : 'www.geogebra.org';
    ...
    applet.removeExistingApplet = function (appletParent, showScreenshot) {
      ...
      for (i = 0; i < appletParent.childNodes.length; i++) {
        ...
        if (className === 'applet_screenshot') {
          ...
          removedID =
            className && className.indexOf('appletParameters') != -1 ? currentChild.id : null;
          i--;
        }
      }
    ...
    var injectHTML5Applet = function (appletElem, parameters, noPreview) {
      ...
      var oriAppletOnload =
        typeof parameters.appletOnLoad === 'function' ? parameters.appletOnLoad : function () {};
      ...
      if (loadScript) {
        ...
        if (
          ...
          (!html5OverwrittenCodebaseVersion || html5OverwrittenCodebaseVersion == '5.0')
        ) {
    ...
    var injectScreenshot = function (appletElem, parameters, showPlayButton) {
      ...
      previewPositioner.className =
        'applet_screenshot applet_scaler' + (showPlayButton ? ' applet_screenshot_play' : '');
      previewPositioner.appendChild(previewContainer);
    ...
    var getPlayButton = function () {
      ...
      if (!window.GGBT_wsf_view) {
        var css =
          ...
          '   background-image: url("static/GeoGebra/images/worksheet/icon-start-applet.png");' +
          '}' +
          '.icon-applet-play:hover {' +
          'background-image: url("static/GeoGebra/images/worksheet/icon-start-applet-hover.png");' +
          '}';
    ...
    var setDefaultHTML5CodebaseForVersion = function (version, offline) {
      ...
      if (index > 0) {
        ...
        codebase = 'static/GeoGebra/apps/5.2.836.0/';
      }
    ...
    var setHTML5CodebaseInternal = function (codebase, offline) {
      ...
      if (html5CodebaseVersion.substr(0, 4) === 'test') {
        html5CodebaseVersion =
          html5CodebaseVersion.substr(4, 1) + '.' + html5CodebaseVersion.substr(5, 1);
      } else if (
  ...
  GGBAppletUtils.makeModule = function (name, permutation) {
    function webModule() {
      ...
      A();
      webModule.__moduleBase = 'static/GeoGebra/apps/5.2.836.0/' + name + '/';
  ...
  if (typeof window.web3d !== 'function') {
    window.web3d = GGBAppletUtils.makeModule('web3d', 'FB5FA39928FEA31DDFDF7F26261E0895');
  }
  if (typeof window.webSimple !== 'function') {
    window.webSimple = GGBAppletUtils.makeModule('webSimple', '40EF93F99E0A5B43E46ECABBF1ED1848');
  }
